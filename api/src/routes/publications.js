  ...
  try {
    ...
    const datasets = await pool.query(
      `SELECT d.id, d.title, d.equipment, e.id AS experiment_id, e.title AS experiment_title,
              (SELECT COUNT(*) FROM dataset_rows WHERE dataset_id = d.id) AS point_count,
              (SELECT COALESCE(json_agg(json_build_object('compound_index', compound_index, 'name', name, 'cas_number', cas_number) ORDER BY compound_index), '[]'::json)
...
router.get('/:id/available-datasets', requireAuth, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT d.id, d.title, d.equipment, e.id AS experiment_id, e.title AS experiment_title,
              (SELECT COUNT(*) FROM dataset_rows WHERE dataset_id = d.id) AS point_count,
              (SELECT COALESCE(json_agg(json_build_object('name', name) ORDER BY compound_index), '[]'::json)
               FROM dataset_compounds WHERE dataset_id = d.id) AS compounds
       FROM datasets d
       JOIN experiments e ON e.id = d.experiment_id
       WHERE d.id NOT IN (
